...
> **坐标**: `com.ecat:integration-serial`
> **版本**: 3.1.0
> **依赖**: `ecat-core ^3.1.0`（provided）

## 概述

为其他集成提供串口的控制访问功能（L2 传输 SDK 层的 serial 域，能力面：主动轮询 `SerialPolling` / 命令与轮询事务 `SerialTransactionStrategy` / 被动监听 `SerialDataListener` / send-receive 策略 `ByteResponseHandlerStrategy`）。
...
- **何时用哪个模式**：周期读设备 → 本 SDK；从机/被动接收（biaoqi 型）→ `SerialSource` onFrame/监听器；写命令 → `executeWithLambda`（命令属性写闸路径最终落到它，有限等待语义保留，见下文「事务入口三选一」）。
...
private Boolean processResponse(ByteResponseHandlingContext<byte[]> context) {
    if (context == null) {
        return false; // 无效响应（正常路径不会发生：checkByteResponse 判定完整帧后才回调）
    }
...
// 发送命令并处理响应：命令事务入口内「直发 + handleResponse 等帧」
// （对齐 src/test .../bytes/ByteCommunicationExample.java 的 demonstrateResponseHandling）
// 发送 API 是 asyncSendData(byte[])；handleResponse 内部注册池化监听器接收字节，
// checkByteResponse 判定完整帧后驱动 processResponse，超时/异常走 handleException。
CompletableFuture<Boolean> result = SerialTransactionStrategy.executeWithLambda(
    serialSource,
    source -> source.asyncSendData(commandBytes)
        .thenCompose(v -> strategy.handleResponse(
            new ByteResponseHandlingContext<byte[]>(commandBytes)))
);
...
批量压测脚本（循环创建 20 对虚拟串口 + 权限/清理）已移至 [docs/perf-test-environment.md](docs/perf-test-environment.md)——日常上手用上方单对 recipe 即可，勿在共享环境跑批量脚本（清理命令含 `pkill`，会误杀无关 socat 进程）。
...
<dependency>
    ...
    <artifactId>integration-serial</artifactId>
    <version>3.1.0</version>
    <scope>provided</scope>
</dependency>
```

在 `ecat-config.yml` 中声明依赖：
