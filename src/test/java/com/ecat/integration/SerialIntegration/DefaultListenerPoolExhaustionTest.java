...
/**
 * 串口 Listener 池耗尽问题复现测试 (DefaultResponseHandlerStrategy)
 *
 * 目标：复现 DefaultResponseHandlerStrategy 在超时情况下 listener 泄漏的问题
 *
 * 问题根因分析：
 * 1. 超时任务执行时释放 listener (DefaultResponseHandlerStrategy:151)
 * 2. whenComplete 始终执行，再次释放 listener (DefaultResponseHandlerStrategy:169)
 * 3. 重复释放导致 listener 无法正确归还到 ThreadLocal 池的可用队列
 *
 * 测试场景：
 * - 场景1：单线程连续超时，快速复现池耗尽，如果server延迟600ms返回，而client500ms超时，导致下一次收到上一次的脏数据，现象是成功一次失败一次。如果都是500ms则全部超时
 * - 场景2：单线程持续异常的兼容处理
 *
 * 前置条件：
 ...
 *
 */
public class DefaultListenerPoolExhaustionTest {

    private static final Log log = LogFactory.getLogger(DefaultListenerPoolExhaustionTest.class);

    // ==================== 常量配置 ====================
    private static final int POOL_SIZE = 20;  // SerialDataListenerPool 的配置大小
    private static final int READ_TIMEOUT_MS = 500;  // Const.READ_TIMEOUT_MS
    private static final int SERVER_DELAY_MS = 505;  // Server 响应延迟，确保超时
    ...
    private static void printPoolStatus(String phase) {
        int available = SerialDataListenerPool.getAvailableCount();
        int inUse = SerialDataListenerPool.getInUseCount();
        int total = SerialDataListenerPool.getTotalCount();
    ...
    public static void main(String[] args) throws Exception {
        ...
        printSeparator('=', 70);
        System.out.println(logWithTimestamp("=== 串口 Listener 池耗尽问题复现测试 (DefaultResponseHandlerStrategy) ==="));
        printSeparator('=', 70);
     ...
     * - Server 延迟 500ms 响应，确保每次都超时（READ_TIMEOUT_MS = 500ms）
     * - 预期：在 20-25 次请求后出现 "No pooled listener available"
     */
    private static void runSingleThreadHighFrequencyTest(SerialSource clientSource)
        ...
        for (int i = 1; i <= REQUEST_COUNT; i++) {
            ...
            // 如果池已耗尽，记录
            int availableNow = SerialDataListenerPool.getAvailableCount();
            if (availableNow == 0) {
    ...
    private static void runExceptionSafetyTest(SerialSource clientSource)
        ...
        System.out.println("  请求次数: " + REQUEST_COUNT);
        System.out.println("  请求间隔: " + REQUEST_INTERVAL_MS + "ms");
        System.out.println("  测试场景: processResponseFunction 抛出异常");
        ...
        for (int i = 1; i <= REQUEST_COUNT; i++) {
            ...
            try {
                // 执行请求，processResponseFunction 会抛异常
    ...
    private static boolean executeRequest(SerialSource clientSource, int requestId, String scenario)
        ...
        // 创建 DefaultResponseHandlerStrategy
        DefaultResponseHandlerStrategy<String> strategy = new DefaultResponseHandlerStrategy<>(
            ...
            response -> {
                // 检查响应完整性
                return response.contains("RESP") && response.contains("ACK") ? response : null;
            },
            ex -> {
                // 处理异常
        ...
        // 创建上下文
        ResponseHandlingContext<String> context = new ResponseHandlingContext<>("req_" + requestId);

        // 使用 SerialTransactionStrategy 执行事务
        CompletableFuture<Boolean> result = SerialTransactionStrategy.executeWithLambda(
            clientSource,
            source -> source.asyncSendData(command)
    ...
    /**
     * 执行单次请求（processResponseFunction 会抛异常）
     *
     * @return true=成功, false=失败
     * @throws RuntimeException processResponseFunction 故意抛出的异常
     */
    private static boolean executeRequestWithExceptionInHandler(SerialSource clientSource, int requestId, String scenario)
        ...
        // 创建 DefaultResponseHandlerStrategy，processResponseFunction 会抛异常
        DefaultResponseHandlerStrategy<String> strategy = new DefaultResponseHandlerStrategy<>(
            clientSource,
            ctx -> {
                // 处理响应
            ...
            ex -> {
                ...
                throw new RuntimeException("TEST: Exception in processResponseFunction");
            }
        );
        ...
        try {
            ...
            // 捕获 ExecutionException，检查其 cause
            Throwable cause = e.getCause();
            if (cause instanceof RuntimeException &&
                cause.getMessage() != null &&
                cause.getMessage().contains("TEST: Exception in processResponseFunction")) {
                throw (RuntimeException) cause;  // 重新抛出原始异常，让上层统计
            }
            throw e;
        }
    }
    ...
    /**
     * 设置 Server 端延迟响应器
     * 延迟 SERVER_DELAY_MS (500ms) 才响应，确保客户端超时
    ...
    private static void printFinalReport() {
        ...
        System.out.println("  可用: " + SerialDataListenerPool.getAvailableCount() + "/" + POOL_SIZE);
        System.out.println("  使用中: " + SerialDataListenerPool.getInUseCount());
        System.out.println("  总计: " + SerialDataListenerPool.getTotalCount());
        System.out.println();
        ...
        if (poolExhaustionCount.get() > 0 || tempListenerUsedCount.get() > 0) {
            ...
            System.err.println("    请检查 DefaultResponseHandlerStrategy 的超时处理逻辑");
        } else {
